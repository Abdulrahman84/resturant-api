...
exports.getCategoryById = async (req, res, next) => {
  ...
  if (!id) {
    return res
      .status(400)
      .json({ message: "must add name of category do you want it" });
  }
  try {
    ...
    if (!category) {
      return res
        .status(200)
        .json({ message: "can't find category with this name" });
    }
...
exports.updateCategoryName = async (req, res, next) => {
  ...
  if (!id) {
    return res
      .status(400)
      .json({ message: "can't update category without id" });
  }
  try {
    ...
    if (name === updateCategoryName.name) {
      return res
        .status(400)
        .json({ message: "The old name is the same as the new name" });
    }
    const updateCategoryNameInProducts = await Product.updateMany(
...
exports.deleteCategory = async (req, res, next) => {
  ...
  if (!id) {
    return res
      .status(400)
      .json({ message: "you can't delete category without id" });
  }
  try {
    ...
    if (deleteCategory) {
      ...
      if (products && deleteProducts) {
        return res
          .status(200)
          .json({
            message: "All product of this category deleted successfully",
          });
      }
      res.status(500).json({ message: "something went wrong" });
    }
