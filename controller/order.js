...
class OrdersAnylisis {
  ...
  recieveOrders() {
    ...
    this.orders.forEach((order) => {
      if (order.recieve == "pick up") pickUpOrders.push(order);
      else deliveryOrders.push(order);
    });
    return {
      pickUpOrders,
  ...
  statusOrders() {
    ...
    let chargedOrders = [];
    this.orders.forEach((order) => {
      if (order.status == "prepared") preparedOrders.push(order);
      else chargedOrders.push(order);
    });
    return {
      preparedOrders,
  ...
  userOrders() {
    ...
    let nonUsersOrders = [];
    this.orders.forEach((order) => {
      if (order.user !== null) usersOrders.push(order);
      else nonUsersOrders.push(order);
    });
    return {
      usersOrders,
...
exports.getClientOrder = async (req, res, next) => {
  ...
  if (req.user) {
    let userOrder = await Order.findOne({ user: req.user })
    ...
    let ip = req.ip;
    let ipOrder = await Order.findOne({ ip })
      .populate("orderProducts")
      .select("adress recieve status orderProducts id");
    if (ipOrder) {
      return res.status(200).json({
        order: ipOrder,
      });
  ...
  await stripe.charges
    ...
    .then(async (result) => {
      try {
        ...
        let deleteOrderProducts = await OrderProducts.deleteMany({
...
exports.deleteOrder = async (req, res, next) => {
  ...
  try {
    ...
    if (deletingOrder) {
      let deletingOrderProducts = await OrderProducts.deleteMany({
        _id: { $in: deletingOrder.orderProducts },
      });
